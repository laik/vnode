...
import (
	testutil "github.com/laik/vnode/pkg/test/util"
	"gotest.tools/assert"
...
func TestGetPods(t *testing.T) {
	...
	rm, err := NewResourceManager(podLister, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetSecret(t *testing.T) {
	...
	rm, err := NewResourceManager(nil, secretLister, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetConfigMap(t *testing.T) {
	...
	rm, err := NewResourceManager(nil, nil, configMapLister, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestListServices(t *testing.T) {
	...
	// Create a new instance of the resource manager based on the pod lister.
	rm, err := NewResourceManager(nil, nil, nil, serviceLister)
	if err != nil {
		t.Fatal(err)
	}
